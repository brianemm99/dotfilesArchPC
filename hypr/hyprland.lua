...
-- Screenshots with grim/slurp -> wl-clipboard -> ~/photos/screenshots
-- fullscreen
hl.bind(mainMod .. "+ Period", hl.dsp.exec_cmd('grim - | tee "$HOME/photos/screenshots/$(date +%Y%m%d_%H%M%S).png" | wl-copy && notify-send "Screenshot" "Saved and copied to clipboard"'))
-- active window
hl.bind(mainMod .. " + ALT + Period", hl.dsp.exec_cmd('grim -g "$(hyprctl activewindow -j | jq -r \'"\\(.at[0]),\\(.at[1]) \\(.size[0])x\\(.size[1])"\')" - | tee "$HOME/photos/screenshots/$(date +%Y%m%d_%H%M%S).png" | wl-copy && notify-send "Screenshot" "Saved and copied to clipboard"'))
-- region
hl.bind(mainMod .. " + SHIFT + Period", hl.dsp.exec_cmd('grim -g "$(slurp -d)" - | tee "$HOME/photos/screenshots/$(date +%Y%m%d_%H%M%S).png" | wl-copy && notify-send "Screenshot" "Saved and copied to clipboard"'))
